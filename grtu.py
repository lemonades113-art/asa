...
class ChatInterface:
    ...
    def chat(self, message: str, thread_id: str):
        """✨ 流式处理用户消息并实时返回响应
        返回：(thought_process, final_report, user_profile, chart_image)
        """
        if thread_id not in self.user_sessions:
            yield "错误：会话不存在，请重新初始化", "", "{}", None
            return
        
        config = self.user_sessions[thread_id]
        thought_buffer = ""  # 思考过程
        report_buffer = ""  # 最终报告
        chart_image_path = None  # 图表路径
        
        try:
            # 1. 初始提示
            thought_buffer += "🚀 **系统启动**，正在分析您的需求...\n\n"
            yield thought_buffer, report_buffer, "{}", None
            ...
            for event in self.app.stream(
                ...
                if next_step == "FINISH":
                    ...
                    yield thought_buffer, report_buffer, "{}", chart_image_path
                ...
                # === 提取 Coder 生成的图表 ===
                if current_sender == "Coder" and messages and len(messages) > 0:
                    last_msg = messages[-1]
                    if hasattr(last_msg, 'content') and last_msg.content:
                        ...
                        if "[IMAGE]:" in msg_content:
                            ...
                            for line in lines:
                                if "[IMAGE]:" in line:
                                    chart_image_path = line.replace("[IMAGE]:", "").strip()
                                    print(f"[DEBUG] 捕获到图表路径：{chart_image_path}")
                                    break
            ...
            # ✅ 在最后一次 Yield 中，输出保存的最新报告内容和图表
            if latest_report:
                report_buffer = latest_report
            
            # 最后一次 Yield，更新画像和图表
            yield thought_buffer, report_buffer, profile_info, chart_image_path
        
        except Exception as e:
            import traceback
            error_msg = f"\n\n❌ **系统错误**: {str(e)}\n\n```\n{traceback.format_exc()}\n```"
            yield thought_buffer + error_msg, "", "{}", None
    ...
    with gr.Blocks(title="A股数据分析助手 2.0") as demo:
        ...
        def on_send_message(message, thread_id_val):
            if not thread_id_val:
                yield "错误: 请先初始化会话", "", "{}", None
                return
            
            # 直接调用生成器函数
            for thought, report, profile, chart_path in chat_interface.chat(message, thread_id_val):
                # 如果chartpath不为None，需要加载图表
                chart_image = None
                if chart_path:
                    try:
                ...
                yield thought, report, profile, chart_image
        ...
        send_btn.click(
            ...
            outputs=[thought_process, final_report, profile_display, chart_display]
        )
